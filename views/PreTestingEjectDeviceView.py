# -*- coding: utf-8 -*-
"""
PreTestingEjectDeviceView - Pre-Testing Eject Device 단계 화면 (Calibration/QC 공통)
"""
import sys
import os
...
class PreTestingEjectDeviceView(QMainWindow):
    ...
    def update_ui_texts(self):
        ...
        if hasattr(self, 'label_eject_title'):
            self.label_eject_title.setText(self.config.get('eject_title', '📤 Remove Device'))
        
        # Eject 지시사항 업데이트
        if hasattr(self, 'label_eject_instruction'):
            self.label_eject_instruction.setText(self.config.get('eject_instruction', 'Please remove the device.'))
        
        # 윈도우 타이틀 업데이트
        self.setWindowTitle(f"{self.config.get('window_title_suffix', 'Pre-Testing')} - Eject Device")
    ...
    def update_kit_info(self):
        ...
        if self.data and 'current_kit' in self.data:
            ...
            if current_kit <= len(kit_names):
                ...
                # 지시사항 업데이트  
                if hasattr(self, 'label_eject_instruction'):
                    # Pre-Testing 타입에 따라 다음 동작 결정
                    if self.pretest_type == PretestConfig.TYPE_QC:
                        next_action = "Insert next QC kit" if current_kit < total_kits else "View QC results"
                        kit_type = "QC"
                    else:
                        next_action = "Insert next calibration kit" if current_kit < total_kits else "View calibration results"
                        kit_type = "calibration"
                    
                    instruction_text = f"""Please remove the {kit_name} {kit_type} kit from the device.
...
if __name__ == "__main__":
    ...
    window = PreTestingEjectDeviceView()
    window.show()
    sys.exit(app.exec_())
