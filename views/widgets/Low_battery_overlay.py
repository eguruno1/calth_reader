...
class LowBatteryOverlayWidget(QWidget):
    """
    ⚠ Low Battery 전역 안내 위젯
    ...
    def show_warning(self):
        """
        🔔 Low Battery 조건 만족 시 호출
        - 이미 열려 있으면 무시
        - 화면 정중앙에 표시
        - 전체 화면 입력 차단
        """
        if self._is_open:
            print("[LowBatteryOverlay] already open → ignore")
            return

        print("[LowBatteryOverlay] SHOW")
        self._is_open = True
    ...
    def on_ok_clicked(self):
        ...
        사용자가 경고를 확인하고 닫을 때
        """
        print("[LowBatteryOverlay] OK clicked")
        self.hide_warning()