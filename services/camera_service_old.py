...
class CameraService(QObject):
    ...
    def get_current_frame(self):
        """현재 프레임 가져오기 (항상 CameraFrame 반환)"""

        print(f"[Camera Service] get_current_frame() self._is_debug_mode: {self._is_debug_mode}")

        if self._is_debug_mode or not app_config.is_camera_enabled():
            if self.dummy_frame is None:
                self._create_dummy_frame()

            # dummy_frame은 np.ndarray → CameraFrame으로 감싸야 함
            return CameraFrame(
                frame_data=self.dummy_frame.copy(),
                timestamp=datetime.now(),
                frame_id=0,
                image=self.dummy_frame.copy()
            )
        ...
        if current_frame is not None:
            # ❌ copy() 절대 호출하지 않음
            print(f"[Camera Service] get_current_frame() current_frame: {current_frame}")
    ...
    # Argus 반응이 없으면 사용
    #################################################
    def discard_frames(self, duration_sec=1.2):
        """
        AE 안정화를 위해 일정 시간 프레임 버림
        """
        start = time.time()
    ...
    def stop_camera(self):
        print("[CameraService] Stopping camera")
        self.stop_capture()
    ...
    def restart_camera(self):
        print("[CameraService] Restarting camera pipeline")
        self.stop_camera()
        time.sleep(0.5)
        self.start_camera()    